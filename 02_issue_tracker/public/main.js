...
$(() => {
  ...
  $.ajax({
      ...
      success: data => {
          let issues = '';
          data.forEach(issue => {
              const openStatus = issue.open
                  ? 'OPEN'
                  : 'CLOSED';
              const visibility = openStatus === 'CLOSED'
                  ? 'hidden'
                  : '';
              const card = `
        <div class="card card-block col-12 col-md-5 col-xl-3">
          <div class="card-header text-white ${openStatus === 'CLOSED'
                  ? 'bg-lightgreen'
                  : 'bg-danger'}">
            <h4 class="card-title">${issue
                      .issue_title}</h4>
            <p><small>Created by: ${issue
                      .created_by}</small></p>
            <b>${openStatus}</b>
          </div>
          <div id="card-body" class="card-body text-left">
            ...
            <p>${issue
                      .issue_text}<p/>
            <hr/>
            <p><small><b>ID:</b> ${issue
                      ._id}</small></p>
            <hr/>
            <p><small><b>Created on:</b><br/> ${moment(issue.created_on, "x")
                      .format("DD MMM YYYY hh:mm a")}</small></p>
            <p><small><b>Updated on:</b><br/> ${moment(issue.updated_on, "x")
                      .format("DD MMM YYYY hh:mm a")}</small></p>
          </div>
