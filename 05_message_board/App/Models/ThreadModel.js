...
module.exports = class ThreadModel {
  async getTopTenThreads(board) {
    ...
    const listOfThreads = await Thread.find()
      .limit(10)
      .sort("-bumped_on")
      .select(
    ...
    for (let thread in listOfThreads)
      listOfThreads[thread].replies = listOfThreads[thread].replies
        .reverse()
        .slice(0, 3);
  ...
  async getOneThread(board, threadId) {
    const Thread = await Schema.getThreadSchema(board);
    const thread = await Thread.findOne({ _id: threadId }).select(
      "-reported -delete_password -replies.reported -replies.delete_password"
    );
    return thread;
  }
  ...
  async getThreadHash(board, threadId) {
    const Thread = await Schema.getThreadSchema(board);
    const hash = await Thread.findOne({ _id: threadId }).select(
      "delete_password"
    );
    return hash;
  }
