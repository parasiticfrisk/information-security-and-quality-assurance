...
module.exports = class ReplyController {
  ...
  async postReply(req) {
    ...
    if (!text || !delete_password || !thread_id)
      throw {
        code: 400,
        text: '"thread_id, "text" and "delete_password" are required.'
      };
    const crypticPasswd = await this.hashPasswd(delete_password);
    const replyPosted = await this.replyModel.saveReply(
      board,
      thread_id,
      text,
      crypticPasswd
    ...
    if (!replyPosted)
      throw {
        code: 404,
        text: `Sorry. Either the board "${board}" or the thread_id "${thread_id}" was not found.`
      };
    return `Reply "${
      replyPosted.replies[replyPosted.replies.length - 1].text
    }" successfully posted.`;
  }
  ...
  async reportReply(req) {
    ...
    const reportedReply = await this.replyModel.reportOneReply(
      board,
      thread_id,
      reply_id
    );
  ...
  async deleteReply(req) {
    ...
    if (!thread_id || !reply_id || !delete_password)
      throw {
        code: 400,
        text: '"thread_id, "reply_id" and "delete_password" are required.'
      };
    ...
    if (isAuthorized) {
      const deletedThread = await this.replyModel.deleteOneReply(
        board,
        thread_id,
        reply_id
      );
