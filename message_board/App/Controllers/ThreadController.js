...
module.exports = class ThreadController {
  ...
  async getTopTenThreads(req) {
    ...
    if (topTen.length === 0)
      throw {
        code: 400,
        text: `Sorry, the board "${req.params.board}" is empty.`
      };
    return topTen;
  }
  ...
  async getThread(req) {
    ...
    const thread = await this.threadModel.getOneThread(
      req.params.board,
      req.query.thread_id
    );
    if (!thread)
      throw {
        code: 404,
        text: `Sorry, but the thread ID was not found. Try again with a different ID.`
      };
    return thread;
  }
  ...
  async postThread(req) {
    ...
    if (!text || !delete_password)
      throw {
        code: 400,
        text: 'Parameters "text" and "delete_password" are required.'
      };
    const crypticPasswd = await this.hashPasswd(delete_password);
    const threadPosted = await this.threadModel.saveThread(
      board,
      text,
      crypticPasswd
    );
  ...
  async reportThread(req) {
    ...
    if (!thread_id) throw { code: 400, text: '"thread_id" is required.' };
    const reportedThread = await this.threadModel.reportOneThread(
      board,
      thread_id
    );
  ...
  async deleteThread(req) {
    ...
    if (!thread_id || !delete_password)
      throw {
        code: 400,
        text: '"thread_id and "delete_password" are required.'
      };
    ...
    if (isAuthorized) {
      const deletedThread = await this.threadModel.deleteOneThread(
        board,
        thread_id
      );
      return "success";
